...
class Config:
    ...
    OUTPUT_DIR = r"./training_output_fpn"
    MODEL_VERSION = "ConvNext_FPN_multiscale"
...
class FullModel(nn.Module):
    ...
    def forward(self, x):
        ...
        # features is now a list of feature maps at different scales
...
def save_sample_predictions(model, val_loader, device, epoch, save_dir, num_samples=3):
    ...
    for i in range(num_samples):
        ...
        # Calculate error using CPU tensors
        error = torch.dist(final_coords[i].cpu(), target_coords[i].cpu())
        axes[i, 0].set_title(f'Image {i+1}: Error={error:.1f}px')
        axes[i, 0].legend()
        axes[i, 0].axis('off')
