...
def validate(model, dataloader, criterion, device, epoch):
    ...
    with torch.no_grad():
        for images, target_heatmaps, target_coords in progress_bar:
            ...
            # Fix: squeeze only the channel dimension to preserve batch dimension
            weight = (1.0 - uncertainty.squeeze(-1))  # [B] shape
            
            # Ensure weight has proper shape for broadcasting
            if len(weight.shape) == 0:  # Handle scalar case (batch_size=1)
                weight = weight.unsqueeze(0)
            
            # Apply weighting with proper broadcasting
            ...
            # Handle uncertainty for logging
            if uncertainty.numel() == 1:
                all_uncertainties.append(uncertainty.cpu().item())
            else:
