...
class MissingBallDataset(Dataset):
    """Dataset for missing ball prediction task."""
...
class ContextAggregator(nn.Module):
    ...
    def forward(self, features):
        ...
        if Config.USE_SELF_ATTENTION:
            ...
            # Fuse contexts
            fused = self.context_fusion(attn_out)
        else:
...
class ContextAwareLoss(nn.Module):
    """Loss function that considers prediction uncertainty."""
    ...
    def forward(self, pred_heatmaps, target_heatmaps, pred_coords, target_coords, uncertainty):
        ...
        weighted_h_loss = (h_loss * confidence).mean()
        weighted_c_loss = (c_loss * confidence).mean()
        
        # Total loss
        total_loss = (self.heatmap_weight * weighted_h_loss + 
                     self.coord_weight * weighted_c_loss + 
                     self.uncertainty_weight * u_loss)
        
        return total_loss, h_loss, c_loss, u_loss
...
def main():
    ...
    # Create transforms - lighter augmentation to preserve context
