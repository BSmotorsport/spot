...
class SSDImageCache:
    ...
    def get_cache_path(self, filename, target_size, quality='high'):
        ...
        cache_path = os.path.join(self.cache_dir, f"{size_hash}.png")
        
        # Store mapping between original filename and cache filename
        map_key = f"{filename}_{size_str}"
        self.filename_map[map_key] = size_hash + ".png"
    ...
    def get_cached_image(self, original_path, target_size, quality='high'):
        ...
        padded_image.save(cache_path, 'PNG', quality=95)
...
def build_grouped_split_files(image_folder: str, seed: int, val_frac: float, expected_group_size: Optional[int] = None) -> Tuple[List[str], List[str]]:
    all_files = sorted([f for f in os.listdir(image_folder) if f.lower().endswith(('.jpg', '.jpeg', '.png'))])
...
def verify_dataset_filenames(data_folder):
    ...
    all_files = [f for f in os.listdir(data_folder) if f.lower().endswith(('.jpg', '.jpeg', '.png'))]
...
if __name__ == "__main__":
    ...
    if args.mode == 'download':
        ...
        data_folder = "E:/botb/dataset/aug"
        verify_dataset_filenames(data_folder)
        ...
        train_model(resume_from=args.resume, original_image_size=original_image_size)
